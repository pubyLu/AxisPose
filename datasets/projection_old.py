...
class Projection(VisionDataset):
    def __init__(self, txtPath, INPUT_H, INPUT_W,transforms = None, target_transform=None):
        ...
        self.H = INPUT_H
        self.W = INPUT_W
        self.img_path, self.mask_path, self.axis_mask_path = self.processTxt()
    def processTxt(self):
        seqs = []
        ...
        for seq_path in seqs:
            ...
            for img in os.listdir(os.path.join(seq_path, "rgb")):
                name = img.split('.')[0]
                tile = img.split('.')[1]
                if tile in ["png", "jpg", "jpeg"]:
                    img_path.append(os.path.join(seq_path,'rgb',img))
                    mask_path.append(os.path.join(seq_path,'mask_visib',name+"_000000.png"))
                    axis_mask_path.append(os.path.join(seq_path, 'axis_masks',name+".png"))
                else:
                    continue
        return img_path, mask_path, axis_mask_path
    def __len__(self):
