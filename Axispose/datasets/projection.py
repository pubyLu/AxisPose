...
class ProjectionSample(VisionDataset):
    def __init__(self, samplePath, INPUT_H, INPUT_W,transforms = None, target_transform=None):
        super(ProjectionSample).__init__()
        self.txtPath = samplePath
        self.transform = transforms
        self.target_transform = target_transform
        self.H = INPUT_H
        self.W = INPUT_W
        self.img_path, self.mask_path, self.names = self.processTxt()
    def processTxt(self):

        img_path=[]
        mask_path=[]
        names =[]
        seqs = os.listdir(os.path.join(self.txtPath, "rgb"))
        for img in seqs:
            name = img.split(".")[0]
            img_path.append(os.path.join(self.txtPath, "rgb", img))
            mask_path.append(os.path.join(self.txtPath, 'mask', f"{name}.png"))
            names.append(name)

        return img_path, mask_path, names
    def __len__(self):
        return len(self.img_path)
